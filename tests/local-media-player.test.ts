...
describe("local media player", () => {
  ...
  it("dashboard exposes local media playlist UI and ended auto-start", () => {
    ...
    expect(html).not.toMatch(/id="localMediaVideo"[^>]*\scontrols/);
    expect(html).not.toMatch(/id="localMediaAudio"[^>]*\scontrols/);
    expect(html).toContain('id="localMediaTransportWrap"');
