...
**安全提示：** 密钥勿公开分享；导出云端配置**不含**密钥，换电脑需手动再填。
...
1. 旧浏览器 **导出云端配置**（含客户端 ID、密钥与登录状态；须已填写密钥并完成长期授权）

2. 新浏览器 **导入云端配置** — 通常**无需再点「连接 Google 账号」**即可上传

3. 若导入后提示登录无效：在 Google 账号设置中撤销过应用权限，或 refresh token 已失效 → 再点 **连接 Google 账号**

**安全：** 配置文件含密钥与登录令牌，请勿分享或上传到公开位置。
